...
def process_product(product_id, conn):
    ...
    try:
        result = run_forecast(df, method='multiplicative', forecast_periods=40)
...
def main():
    print("="*60)
    print("FORECASTING SYSTEM - Multiplicative Decomposition")
    print("="*60)
    print("\nStarting batch forecast process for all products...")
    ...
    print(f"\nNote: MPS values are calculated on frontend using:")
    print(f"  - Forecast ÷ variant.number = demand per pack")
    print(f"  - MPS = Forecast - Projected On Hand (previous week)")
    print(f"  - Available = Available (prev) + MPS - Forecast")
    print(f"{'='*60}")
