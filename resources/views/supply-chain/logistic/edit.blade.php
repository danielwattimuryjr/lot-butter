...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form method="POST" action="{{ route('employee.supply-chain.logistics.update', $logistic) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="component_id" id="component" class="w-full px-4 py-3 bg-gray-100 border-0 rounded-lg focus:ring-2 focus:ring-butter-400 focus:bg-white transition-all outline-none" required>
                  ...
                  @foreach ($components as $component)
                    <option value="{{ $component->id }}" {{ old('component_id', $logistic->component_id) == $component->id ? 'selected' : '' }}>
                      {{ $component->name }} 
                    </option>
                  @endforeach
                </select>
            ...
            <div>
                ...
                <textarea class="w-full px-4 py-3 bg-gray-100 border-0 rounded-lg focus:ring-2 focus:ring-butter-400 focus:bg-white transition-all outline-none" name="notes" id="notes">{{ old('notes', $logistic->notes) }}</textarea>
                @error('notes')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
