    ...
    <div class="space-y-6">
        ...
        @if (auth()->user()->team->name == "Production")
            ...
            <div class="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
                ...
                <a
                    href="{{ route("employee.production.products.bill-of-materials.create", $product) }}"
                    ...
                    <tbody>
                        @forelse ($product->components as $component)
                            <tr class="border-b border-gray-100 hover:bg-gray-50">
                                ...
                                <td class="px-4 py-4 text-sm text-gray-700">{{ $component->pivot->quantity }}</td>
                                <td class="px-4 py-4 text-sm text-gray-700">{{ $component->unit }}</td>
                                @if (auth()->user()->team->name == "Production")
                                    <td class="px-4 py-4">
                                        <div class="flex items-center gap-3">
                                            <a
                                                href="{{ route("employee.production.products.bill-of-materials.edit", [$product, $component]) }}"
                                            ...
                                            <form
                                                method="POST"
                                                action="{{ route("employee.production.products.bill-of-materials.destroy", [$product, $component]) }}"
